...
class Hangman extends Component {
  ...
  */
  ...
  resetgame() {
    this.setState({ answer: randomWord(), nWrong: 0, guessed: new Set() });
  }
  /** render: render game */
  render() {
    let gameover = this.props.maxWrong > this.state.nWrong;
    let win = this.state.answer === this.guessedWord().join("");
    console.log(this.state.answer, this.guessedWord().join(""));
    console.log(win);
    return (
